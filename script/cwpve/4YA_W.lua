    ...
    elseif _event.id == world.event.S_EVENT_TAKEOFF then
        ...
        local _unit = _event.initiator
         ...
         --   end
            ...
            -- warning
            local _groupId = dcsprotect.getGroupId(_unit)
            trigger.action.outTextForGroup(_groupId, "**[4YA] Cold War PVE** \n\ \n\All airports that needs to be captured have markers on the F10 map (list in briefing). \n\If the airport has no friendly ground units visible on the F10 map, then it is hostile. \n\ \n\Comms menu -> F10 (Others) -> Requst Bogey Dope\n\ \n\Check the briefing for more info!\n\Banned weapons:" .. _list, 15, true)

--[[
	Storm of War WW2 Radar Script - v24 12-Mar-2020, xvii-Dietrich, tested on DCS v2.5.6
	Storm of War WW2 Radar Script - v19.2 PB edit - 12-Mar-2020, xvii-Dietrich, edited by PB, tested on DCS v2.5.6
	Storm of War WW2 Radar Script - v20 Wiggy minimum altitude additions

   based on Steggles Early Warning Radar Script - 1.5.3 - 07/11/2016
...
ewrs.minVisibleAltitude = 126 -- Minimum altitude threats will be detected (metres)
...
function ewrs.buildThreatTable(activePlayer, bogeyDope)
	...
	for k,v in pairs(targets) do
		...
		local bearing = ewrs.getBearing(referenceX,referenceZ,bogeypos.p.x,bogeypos.p.z) -- degrees
		local heading = ewrs.getHeading(velocity) -- degrees
		local range = ewrs.getDistance(referenceX,referenceZ,bogeypos.p.x,bogeypos.p.z) -- metres
		local altitude = bogeypos.p.y --metres
		local speed = ewrs.getSpeed(velocity) --m/s
		
		if altitude > ewrs.minVisibleAltitude then 

			local bearingtext = ewrs.bearingToCardinal(bearing)
			local headingtext = ewrs.bearingToCardinal(heading)
			local rangetext    = ewrs.approxRange(range)
			local altitudetext = ewrs.approxAltitude(altitude)
				...
				threatTable[j].bearingtext = bearingtext
				threatTable[j].range = range
				threatTable[j].rangetext = rangetext
				threatTable[j].altitudetext = altitudetext
				threatTable[j].headingtext = headingtext
		else
		end
	end
		...
		if #threatTable >= 1 then
			...
			if greeting == nil then
				if bogeyDope then
					...
					messageGreeting = "RADAR Report: From your location" 
				end
			else
			...
			table.insert(message, "\n\n")
			table.insert(message, string.format( "%s%8s", "BRG"," "))
			table.insert(message, string.format( "%s%8s", "RNG  "," "))
			table.insert(message, string.format( "%s%8s", "ALT   "," "))
			table.insert(message, string.format( "%s%8s", "HDG"," "))
			table.insert(message, string.format( "%s", "TYPE"))
			table.insert(message, "\n")
			...
			for k = 1, maxThreats do
				...
				if threatTable[k].range == ewrs.notAvailable then
					table.insert(message, string.format( "%s", "  ... Position unknown ...                         "))
				else
					table.insert(message, string.format( "%s%8s", threatTable[k].bearingtext," "))
					table.insert(message, string.format( "%s%8s",  threatTable[k].rangetext," "))
					table.insert(message, string.format( "%s%8s",  threatTable[k].altitudetext," "))
					table.insert(message, string.format( "%s%8s", threatTable[k].headingtext," "))
				table.insert(message, string.format( "%s", threatTable[k].unitType))
				end
				table.insert(message, "\n")
			end
...
function ewrs.buildFriendlyTable(friendlyNames,activePlayer)
	...
	for k,v in pairs(units) do
		...
		if pos.p.x ~= selfpos.p.x and pos.p.z ~= selfpos.p.z then --same position as self, means its you!
         ...
         -- Text variables. Assume the worst case, and update if needed
         local bearingtext = ewrs.bearingToCardinal(bearing)
         local headingtext = ewrs.bearingToCardinal(heading)
         local rangetext    = ewrs.approxRange(range)
         local altitudetext = ewrs.approxAltitude(altitude)
			...
			friendlyTable[j].bearingtext = bearingtext
			friendlyTable[j].range = range
			friendlyTable[j].rangetext = rangetext
			friendlyTable[j].altitudetext = altitudetext
			friendlyTable[j].headingtext = headingtext
		else
	...
	local status, result = pcall(function()
		for i = 1, #ewrs.activePlayers do
			if ewrs.activePlayers[i].groupID == args[1] then
				...
				local greeting = "Friendlies report:"
...
--				local rootPath = missionCommands.addSubMenuForGroup(groupID, "RADAR Controller")
...
--				--end
				...
				if ewrs.onDemand then
					missionCommands.addCommandForGroup(groupID, "RADAR: Detect Hostile Aircraft",nil,ewrs.onDemandMessage,{groupID})
				end
				
				if ewrs.allowFriendlyPicture then
					missionCommands.addCommandForGroup(groupID, "RADAR: Detect Friendly Aircraft",nil,ewrs.friendlyPicture,{groupID})
				end
...
trigger.action.outText("RADAR Controller is available. Use comms menu, F10. -based on steggles EWRS-",15)
